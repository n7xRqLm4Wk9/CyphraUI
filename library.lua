...
-- ─── Dragging Handler Utility (For Window) ───────────────────────────────────
...
function CyphraUI:CreateWindow(settings)
	...
	-- FIX: Anchored Control Panel Actions Layout
	...
	-- Minimize Standalone Toggle Node
	...
	-- FIX: Completely clean drag vs click logic with zero memory leaks
	...
	UserInputService.InputEnded:Connect(function(input)
		if dragActive and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
			dragActive = false
			-- If moved less than 10 pixels, treat as a clean click
	...
	makeControlBtn("×", 44, function() gui:Destroy() end)
	makeControlBtn("−", 10, function()
	...
	function windowObj:CreateTab(name)
		...
		local tabObj = { _page = page, _btn = tabBtn, _order = 0 }
		table.insert(self._tabs, tabObj)

		-- FIX: Scope correction. We now loop over windowObj._tabs reliably.
		...
		function tabObj:CreateToggle(settings)
			...
			local pill = make("Frame", {
				Size = UDim2.new(0, 32, 0, 16), Position = UDim2.new(1, -44, 0.5, -8),
				BackgroundColor3 = current and THEME.Accent or THEME.Card, BorderSizePixel = 0,
			}, card)
			addCorner(pill, 8)

			local knob = make("Frame", {
				Size = UDim2.new(0, 12, 0, 12), Position = current and UDim2.new(0, 18, 0.5, -6) or UDim2.new(0, 2, 0.5, -6),
				BackgroundColor3 = current and THEME.Background or THEME.SubText, BorderSizePixel = 0,
			}, pill)
			addCorner(knob, 6)
		...
		function tabObj:CreateSlider(settings)
			...
			make("TextLabel", {
				Size = UDim2.new(1, -80, 0, 20), Position = UDim2.new(0, 12, 0, 4),
				BackgroundTransparency = 1, Text = name, TextColor3 = THEME.Text,
				Font = Enum.Font.Gotham, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left,
			}, card)

			local valLabel = make("TextLabel", {
				Size = UDim2.new(0, 60, 0, 20), Position = UDim2.new(1, -72, 0, 4),
				BackgroundTransparency = 1, Text = tostring(current), TextColor3 = THEME.SubText,
				Font = Enum.Font.Code, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Right,
			}, card)

			local track = make("Frame", {
				Size = UDim2.new(1, -24, 0, 4), Position = UDim2.new(0, 12, 0, 30),
