--[[
	...
	Optimized for Mobile Layouts & Bug Fixes
--]]
...
function CyphraUI:CreateWindow(settings)
	...
	addCorner(window, 9)
	addStroke(window, THEME.Stroke, 1) -- Matte dark borders instead of white-ish outlines
	...
	-- Rounding top bar cleanly without causing sharp edges to pop out behind it
	local titleCorner = addCorner(titleBar, 9)
	
	-- Fix background leaking line issue
	...
	-- Control Panel Actions Layout (Fixes the right side clip bug in 1000106342.jpg)
	local controlLayout = make("Frame", {
		Size = UDim2.new(0, 70, 1, 0),
		Position = UDim2.new(1, -82, 0, 0), -- Pushed safely to the left to prevent cutting out edges
		BackgroundTransparency = 1,
	}, titleBar)
	...
	-- Create Draggable Floating Minimize Hub Button (Fixes minimize bug)
	...
	addCorner(floatBtn, 25) -- Circle shape button
	...
	function windowObj:CreateTab(name)
		...
		local page = make("ScrollingFrame", {
			...
			BackgroundTransparency = 1,
			BorderSizePixel  = 0,
			ScrollBarThickness = 0, -- Seamless hidden canvas bar track
			Visible          = isFirst,
		...
		function tabObj:CreateDropdown(settings)
			...
			local list = make("Frame", {
				Size = UDim2.new(1, 0, 0, #options * 26), Position = UDim2.new(0, 0, 0, 40),
			...
			click.MouseButton1Click:Connect(function()
				...
				wrap.Size = expanded and UDim2.new(1, 0, 0, 40 + (#options * 26)) or UDim2.new(1, 0, 0, 36)
