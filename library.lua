...
function Luxware:CreateWindow(options)
    ...
    -- Absolute distance check for Mobile Drag vs Tap
    ...
    OpenBtn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if openDragStart then
                local magnitude = (input.Position - openDragStart).Magnitude
                if magnitude < 10 then -- Definitively a tap, not a drag
