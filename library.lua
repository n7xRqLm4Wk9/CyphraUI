...
local Luxware = {
    Flags = {}, -- Stores elements for config saving
}
Luxware.__index = Luxware

-- // Premium AMOLED Theme
local Theme = {
...
-- // Filesystem Checks for Config Saving
...
-- // Fixed Unified Dragging Architecture
...
local NotifyGui = Create("ScreenGui", {
    Name = "LuxwareNotifications", Parent = pcall(function() return CoreGui end) and CoreGui or LocalPlayer:WaitForChild("PlayerGui"), ZIndexBehavior = Enum.ZIndexBehavior.Sibling
})
...
function Luxware:Notification(opts)
    ...
    local AccentLine = Create("Frame", {
        Parent = NotifFrame, BackgroundColor3 = Theme.Accent, Size = UDim2.new(0, 3, 1, -16), Position = UDim2.new(0, 8, 0.5, 0), AnchorPoint = Vector2.new(0, 0.5), BackgroundTransparency = 1
    })
...
-- // Config System
function Luxware:SaveConfig(filename)
    if not hasFilesystem then return end
    pcall(function()
        if not isfolder("LuxwareConfigs") then makefolder("LuxwareConfigs") end
        local data = {}
        for flag, element in pairs(Luxware.Flags) do
        ...
        writefile("LuxwareConfigs/" .. filename .. ".json", HttpService:JSONEncode(data))
        Luxware:Notification({Title = "System", Description = "Configuration saved successfully.", Duration = 2})
    end)
end

function Luxware:LoadConfig(filename)
    if not hasFilesystem then return end
    pcall(function()
        local path = "LuxwareConfigs/" .. filename .. ".json"
        if isfile(path) then
            local data = HttpService:JSONDecode(readfile(path))
            for flag, value in pairs(data) do
                if Luxware.Flags[flag] then
                    if Luxware.Flags[flag].Type == "ColorPicker" then
                        Luxware.Flags[flag]:Set(Color3.new(value.R, value.G, value.B))
                    else
                        Luxware.Flags[flag]:Set(value)
                    end
                end
            end
            Luxware:Notification({Title = "System", Description = "Configuration loaded successfully.", Duration = 2})
        end
    end)
end
...
function Luxware:CreateWindow(options)
    ...
    -- ============================================================
    -- FIXED: OpenBtn Unified Drag + Tap Detection
    -- ============================================================
    local openDragStartPos = nil
    local openDragStartTime = nil
    local openIsDragging = false
    local OPEN_DRAG_THRESHOLD = 8
    local OPEN_TAP_MAX_TIME = 0.4

    local openDragStart, openDragInput, openStartPos

    OpenBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            openDragStartPos = input.Position
            openDragStartTime = tick()
            ...
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    openDragStartPos = nil
                    openDragStartTime = nil
                end
            end)
        end
    end)
    ...
    OpenBtn.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            if openDragStartPos and not openIsDragging then
                local delta = (input.Position - openDragStartPos).Magnitude
                if delta > OPEN_DRAG_THRESHOLD then
                    openIsDragging = true
                end
            end
            openDragInput = input
    ...
    UserInputService.InputChanged:Connect(function(input)
        if openDragStartPos and openIsDragging and input == openDragInput then
    ...
    OpenBtn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if openDragStartPos and not openIsDragging then
                local elapsed = tick() - openDragStartTime
                if elapsed < OPEN_TAP_MAX_TIME then
    ...
    local function BuildElementsAPI(TargetParent, IsNested)
        ...
        function API:CreateButton(opts)
            ...
            Btn.Activated:Connect(function()
                ...
                PlayTween(Btn, {BackgroundColor3 = Theme.Accent, TextColor3 = Theme.Background})
                task.wait(0.15)
                PlayTween(Btn, {BackgroundColor3 = Theme.Sidebar, TextColor3 = Theme.Text})
            end)
        end
        ...
        function API:CreateToggle(opts)
            ...
            opts.Type = "Toggle"
            if opts.Flag then Luxware.Flags[opts.Flag] = opts end

            local Switch = Create("TextButton", {
                Parent = Row, BackgroundColor3 = opts.CurrentValue and Theme.Accent or Theme.Sidebar, Size = UDim2.new(0, 38, 0, 20), Position = UDim2.new(1, -52, 0, 15), Text = "", AutoButtonColor = false
            })
            ...
            local function SetState(state)
                ...
                PlayTween(Switch, {BackgroundColor3 = state and Theme.Accent or Theme.Sidebar})
        ...
        function API:CreateSlider(opts)
            ...
            local Fill = Create("Frame", {
                Parent = Track, BackgroundColor3 = Theme.Accent, Size = UDim2.new(math.clamp((opts.CurrentValue - opts.Range[1]) / (opts.Range[2] - opts.Range[1]), 0, 1), 0, 1, 0)
            })
            ApplyCorner(Fill, 10)
        ...
        -- // Interactive HSV Color Picker (FIXED: hue no longer inverted)
        function API:CreateColorPicker(opts)
            ...
            local HueCursor = Create("Frame", {Parent = HueSlider, BackgroundColor3 = Theme.Background, Size = UDim2.new(0, 4, 1, 4), Position = UDim2.new(0, 0, 0.5, -2), AnchorPoint = Vector2.new(0.5, 0.5)})

            -- Saturation/Value Map
            local SVMap = Create("TextButton", {
                Parent = ExpandPanel, BackgroundColor3 = Color3.fromRGB(255, 0, 0), Size = UDim2.new(1, -28, 0, 70), Position = UDim2.new(0, 14, 0, 35), Text = "", AutoButtonColor = false
            })
            ApplyCorner(SVMap, 4)
            local SatGradient = Create("UIGradient", {Parent = SVMap, Color = ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(255, 255, 255)), Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 1)})})
            local ValOverlay = Create("Frame", {Parent = SVMap, BackgroundColor3 = Color3.fromRGB(0, 0, 0), Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1})
            ApplyCorner(ValOverlay, 4)
            local ValGradient = Create("UIGradient", {Parent = ValOverlay, Rotation = 90, Color = ColorSequence.new(Color3.fromRGB(0, 0, 0), Color3.fromRGB(0, 0, 0)), Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0)})})
            local SVCursor = Create("Frame", {Parent = SVMap, BackgroundColor3 = Theme.Background, Size = UDim2.new(0, 8, 0, 8), AnchorPoint = Vector2.new(0.5, 0.5)})
        ...
        if not IsNested then
            function API:CreateModule(opts)
                ...
                local Switch = Create("TextButton", {
                    Parent = Row, BackgroundColor3 = opts.CurrentValue and Theme.Accent or Theme.Sidebar, Size = UDim2.new(0, 38, 0, 20), Position = UDim2.new(1, -95, 0, 15), Text = "", AutoButtonColor = false
                })
                ApplyCorner(Switch, 10)
                local Ball = Create("Frame", {
                    Parent = Switch, BackgroundColor3 = opts.CurrentValue and Theme.Background or Theme.SubText, Size = UDim2.new(0, 12, 0, 12), Position = opts.CurrentValue and UDim2.new(1, -15, 0.5, -6) or UDim2.new(0, 3, 0.5, -6)
                ...
                Switch.Activated:Connect(function()
                    ...
                    PlayTween(Switch, {BackgroundColor3 = opts.CurrentValue and Theme.Accent or Theme.Sidebar})
